...
if($evote->verifyUser($_SESSION["user"], 0)){
	...
	foreach($codes as $c){
		$count ++;
		$pdf->SetFont('Courier','B',16);
		$title = decode_str(pickLanguage("E-vote, E-sektionens röstningssystem ", "E-vote, E-sektionen's voting system").$_SERVER['SERVER_NAME']);
		$pdf->Ln();
		...
		$pdf->Ln();
		$pdf->Multicell(190,10,decode_str(pickLanguage("Din personliga kod är", "Your personal code is")), 0, 'C', false);
